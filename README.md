...
```json
{
  "templates": ["template_1", "template_2", "template_3", "template_4", "logo"]
}
```
...
|-----------|------|----------|-------------|---------|
| `template` | String | No | Template name from template folder (e.g., "template_1") | - |
| `image` | File | No | Main image to display (if template not used) | - |
...
### Using `curl` with template

```bash
curl -X POST \
  -F "template=template_1" \
